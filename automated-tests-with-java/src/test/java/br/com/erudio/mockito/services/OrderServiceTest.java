...
class OrderServiceTest {
	...
	private final LocalDateTime defaulLocalDateTime = LocalDateTime.of(2023, 7, 4, 15, 50);
	...
	void testShouldIncludeCurrentTime_When_CreateANewOrder() {
		...
		try(MockedStatic<LocalDateTime> mockedLocalDateTime = mockStatic(LocalDateTime.class)){
			...
			mockedLocalDateTime.when(LocalDateTime::now).thenReturn(defaulLocalDateTime);
			
			// When / Act
			Order result = service.createOrder("MacBook Pro", 2L, null);
			
			// Then / Assert
			assertEquals(defaulLocalDateTime, result.getCreationDate());
		}
	}
}
